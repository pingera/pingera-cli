...
• End: [white]{end or 'Not specified'}[/white]"""
    ...
    def get_bulk_uptime(self, page_id: str, start: Optional[str] = None, end: Optional[str] = None):
        ...
        try:
            ...
            if not bulk_uptime:
                if self.output_format in ['json', 'yaml']:
                    self.output_data({"components": [], "total": 0, "message": "No uptime data found"})
                else:
                    self.display_info("No uptime data found.")
                return

            # Prepare data for different output formats
            if self.output_format in ['json', 'yaml']:
                uptime_list = []
                for component_uptime in bulk_uptime:
                    uptime_dict = {
                        "component_id": component_uptime.component_id if hasattr(component_uptime, 'component_id') else None,
                        "component_name": component_uptime.component_name if hasattr(component_uptime, 'component_name') else None,
                        "uptime_percentage": component_uptime.uptime_percentage if hasattr(component_uptime, 'uptime_percentage') else None,
                        "total_time": component_uptime.total_time if hasattr(component_uptime, 'total_time') else None,
                        "downtime": component_uptime.downtime if hasattr(component_uptime, 'downtime') else None,
                        "incidents_count": component_uptime.incidents_count if hasattr(component_uptime, 'incidents_count') else None,
                    }
                    uptime_list.append(uptime_dict)

                self.output_data({
                    "page_id": page_id,
                    "components": uptime_list,
                    "total": len(uptime_list),
                    "start_date": start,
                    "end_date": end,
                })
            else:
                # Create table for default output
                table = Table(title=f"Component Uptime - Page {page_id}")
                table.add_column("Component ID", style="cyan", min_width=15)
                table.add_column("Name", style="green", min_width=20)
                table.add_column("Uptime %", style="yellow", min_width=10)
                table.add_column("Total Time", style="blue", min_width=12)
                table.add_column("Downtime", style="red", min_width=12)
                table.add_column("Incidents", style="magenta", min_width=10)

                for component_uptime in bulk_uptime:
                    component_id = str(component_uptime.component_id) if hasattr(component_uptime, 'component_id') and component_uptime.component_id else "-"
                    name = str(component_uptime.component_name) if hasattr(component_uptime, 'component_name') and component_uptime.component_name else "-"
                    
                    uptime_pct = component_uptime.uptime_percentage if hasattr(component_uptime, 'uptime_percentage') else 0
                    
                    # Color code based on uptime percentage
                    if uptime_pct >= 99.9:
                        uptime_display = f"[green]{uptime_pct:.3f}%[/green]"
                    elif uptime_pct >= 99.0:
                        uptime_display = f"[yellow]{uptime_pct:.3f}%[/yellow]"
                    else:
                        uptime_display = f"[red]{uptime_pct:.3f}%[/red]"
                    
                    total_time = str(component_uptime.total_time) if hasattr(component_uptime, 'total_time') and component_uptime.total_time else "-"
                    downtime = str(component_uptime.downtime) if hasattr(component_uptime, 'downtime') and component_uptime.downtime else "-"
                    incidents = str(component_uptime.incidents_count) if hasattr(component_uptime, 'incidents_count') else "0"
                ...
                self.console.print(table)
                self.console.print(f"\n[dim]Found uptime data for {len(bulk_uptime)} components[/dim]")
                
                if start or end:
                    self.console.print(f"[dim]Period: {start or 'start'} to {end or 'now'}[/dim]")

        except Exception as e:
            self.display_error(f"Failed to get bulk uptime data: {str(e)}")
            raise typer.Exit(1)
